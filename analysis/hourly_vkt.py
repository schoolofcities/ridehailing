...
COLS = [
    ...
    "time_ontrip", "time_enroute", "time_waiting", "time_available",
]
CHUNK_SIZE = 100_000

# Accumulators: one row per hour-of-day (0-23)
dist_acc = np.zeros((24, 3), dtype=np.float64)  # routed, en_route, available
time_acc = np.zeros((24, 4), dtype=np.float64)  # ontrip, enroute, waiting, available
            ...
            for chunk in pd.read_csv(f, usecols=COLS, chunksize=CHUNK_SIZE):
                ...
                for h in range(24):
                    ...
                    if mask.any():
                        ...
                        time_acc[h, 2] += chunk.loc[mask, "time_waiting"].sum()
                        time_acc[h, 3] += chunk.loc[mask, "time_available"].sum()
...
time_summary = pd.DataFrame({
    ...
    "time_waiting":   time_acc[:, 2],
    "time_available": time_acc[:, 3],
})
